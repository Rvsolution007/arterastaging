...
class FestivalAiImageService
{
    public function generate(FestivalAiGeneration $generation): array
    {
        ...
        $editableArtwork = (($generation->request_diagnostics ?? [])['output_mode'] ?? 'flat') === 'editable_v1';
        $brandLogoReference = $editableArtwork ? null : $this->brandLogoReference(
            (array) $generation->business_snapshot,
            (array) $generation->brand_chrome_snapshot
        );
        $brandLogoExpected = !$editableArtwork && $this->brandLogoExpected(
            (array) $generation->business_snapshot,
            (array) $generation->brand_chrome_snapshot
        );
        ...
        // Flat Festival mode owns branding inside provider pixels. Editable V1
        // keeps it in the manifest so it remains selectable and editable.
        $this->updateDiagnostics($generation, [
            'branding_render_mode' => $editableArtwork
                ? 'editable_overlay_manifest'
                : 'provider_prompt_and_reference_image',
            'branding_overlay_requested' => $editableArtwork,
            'branding_overlay_applied' => false,
            'branding_overlay_result' => $editableArtwork
                ? 'deferred_to_editable_document'
                : 'disabled_by_provider_render_mode',
        ]);

...
class _BusinessProfileScreenState extends State<BusinessProfileScreen> {
  ...
  void initState() {
    ...
    List<String> getBizList(String key, List<String> hcVal) {
      if (widget.isNew) return [];
      if (useHc) return hcVal;
      return (biz[key] as List<dynamic>? ?? []).map((e) => e.toString()).toList();
    }

    _nameCtrl = TextEditingController(text: getBizStr('name', 'bussinessName', hc.businessName.value));
    _emailCtrl = TextEditingController(text: getBizStr('email', 'bussinessEmail', hc.businessEmail.value));
    _phoneCtrl = TextEditingController(text: getBizStr('mobileNo', 'mobile_no', hc.businessPhone.value));
    _websiteCtrl = TextEditingController(text: getBizStr('website', 'bussinessWebsite', hc.businessWebsite.value));
    _addressCtrl = TextEditingController(text: getBizStr('address', 'bussinessAddress', hc.businessAddress.value));
    ...
    bool catExists = hc.profileCategories.any((cat) => (cat['businessCategoryId']?.toString() ?? cat['id']?.toString()) == catId);
    ...
    _extraEmailCtrls = extEmails.map((e) => TextEditingController(text: e)).toList();
    _extraPhoneCtrls = extPhones.map((e) => TextEditingController(text: e)).toList();
    _extraWebsiteCtrls = extWebsites.map((e) => TextEditingController(text: e)).toList();
    _extraAddressCtrls = extAddrs.map((e) => TextEditingController(text: e)).toList();
    ...
    for (int i = 0; i < extEmails.length; i++) {
      if (hEmails.contains(extEmails[i])) _hiddenEmails.add(_extraEmailCtrls[i]);
    }
    for (int i = 0; i < extPhones.length; i++) {
      if (hPhones.contains(extPhones[i])) _hiddenPhones.add(_extraPhoneCtrls[i]);
    }
    for (int i = 0; i < extWebsites.length; i++) {
      if (hWebs.contains(extWebsites[i])) _hiddenWebsites.add(_extraWebsiteCtrls[i]);
    }
    for (int i = 0; i < extAddrs.length; i++) {
      if (hAddrs.contains(extAddrs[i])) _hiddenAddresses.add(_extraAddressCtrls[i]);
    }
  }
  ...
  void dispose() {
    ...
    for (var ctrl in _extraEmailCtrls) { ctrl.dispose(); }
    for (var ctrl in _extraPhoneCtrls) { ctrl.dispose(); }
    for (var ctrl in _extraWebsiteCtrls) { ctrl.dispose(); }
    for (var ctrl in _extraAddressCtrls) { ctrl.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _saveProfile() async {
    if (_nameCtrl.text.isEmpty) {
      Get.snackbar('Error', 'Business name is required', backgroundColor: Colors.red, colorText: Colors.white);
      return;
    }
    ...
    if (_hiddenEmails.contains(_emailCtrl) && _emailCtrl.text.isNotEmpty) hiddenEmails.add(_emailCtrl.text);
    for (var c in _extraEmailCtrls) { if (_hiddenEmails.contains(c) && c.text.isNotEmpty) hiddenEmails.add(c.text); }
    
    if (_hiddenPhones.contains(_phoneCtrl) && _phoneCtrl.text.isNotEmpty) hiddenPhones.add(_phoneCtrl.text);
    for (var c in _extraPhoneCtrls) { if (_hiddenPhones.contains(c) && c.text.isNotEmpty) hiddenPhones.add(c.text); }
    
    if (_hiddenWebsites.contains(_websiteCtrl) && _websiteCtrl.text.isNotEmpty) hiddenWebsites.add(_websiteCtrl.text);
    for (var c in _extraWebsiteCtrls) { if (_hiddenWebsites.contains(c) && c.text.isNotEmpty) hiddenWebsites.add(c.text); }
    
    if (_hiddenAddresses.contains(_addressCtrl) && _addressCtrl.text.isNotEmpty) hiddenAddresses.add(_addressCtrl.text);
    for (var c in _extraAddressCtrls) { if (_hiddenAddresses.contains(c) && c.text.isNotEmpty) hiddenAddresses.add(c.text); }

    try {
      final isNewBusiness = widget.isNew || (widget.business == null && hc.businessId.value.isEmpty);
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                  ...
                  Container(
                    ...
                    child: _selectedImage != null
                        ...
                        : (_previewLogoUrl.isNotEmpty
                            ...
                            : Icon(Icons.storefront_outlined, size: 50, color: AppColors.gray400)),
                  ),
                  Positioned(
                    bottom: 0,
                    right: 0,
            ...
            AppSpacing.gapV16,
            _buildDynamicInputFields('Address', Icons.location_on_outlined, _addressCtrl, _extraAddressCtrls, _hiddenAddresses, () {
  ...
  Widget _buildInputField(String label, IconData icon, TextEditingController controller, {TextInputType? keyboardType, int maxLines = 1}) {
    return Column(

...
class _NativeEditorScreenState extends State<NativeEditorScreen> {
  ...
  final RxString _aiCurrentStep =
      'menu'.obs; // 'menu', 'customPrompt', 'review', 'hiring_questionnaire'
  final RxMap<String, TextEditingController> _aiGeneratedTexts =
  ...
  Future<void> _exportAndSave() async {
    ...
    if (!assetsReady) {
      Get.snackbar(
        'Loading...',
        'Some images are still downloading. Please wait a moment.',
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('design_saved_to_gallery'.trFormat),
  ...
  Future<void> captureCanvasSnapshot() async {
    try {
      ...
      NativeEditorController.transitionSnapshot.value = byteData.buffer
          .asUint8List();
      debugPrint(
        '[SNAPSHOT] Captured snapshot size: ${NativeEditorController.transitionSnapshot.value?.length}',
      );
    } catch (e, stackTrace) {
  ...
  void initState() {
    ...
    if (widget.frameData['json'] != null &&
        widget.frameData['json'].toString().trim().isNotEmpty) {
    ...
    if (config.isEmpty ||
        (config['layers'] == null && config['info'] == null)) {
      final imgUrl = widget.designUrl.isNotEmpty
          ? widget.designUrl
          : widget.frameData['image'] ?? '';
      config = {
        'info': {
    ...
    if (templateBaseUrl.isEmpty ||
        !templateBaseUrl.contains('/uploads/template/')) {
      String zipName =
    ...
    if (baseImg.isEmpty && config['layers'] != null) {
      for (var layer in config['layers']) {
        if (layer['is_background'] == true ||
            (layer['is_background'] == null &&
                (layer['name'] == 'bg' || layer['name'] == 'image1'))) {
          baseImg = layer['src'] ?? '';
    ...
    debugPrint(
      '║ [INIT] config.layers.length=${(config['layers'] as List?)?.length ?? 0}',
    );
    if (config['layers'] != null) {
      for (var layer in config['layers']) {
        debugPrint(
          '║ [INIT]   layer: name="${layer['name'] ?? layer['id']}" type="${layer['type']}" src="${layer['src'] ?? 'N/A'}"',
        );
      }
    }
    debugPrint('╚══════════════════════════════════════════════════════');
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
                  ...
                  child: LayoutBuilder(
                    builder: (context, constraints) {
                      return Obx(() {
                        ...
                        debugPrint(
                          '[DIAGNOSIS_CANVAS] Timestamp: $rebuildTime',
                        );
                        debugPrint(
                          '[DIAGNOSIS_CANVAS] Layer Count: ${layers.length}',
                        );
                        debugPrint('[DIAGNOSIS_CANVAS] Layer IDs: $layerIds');
                        debugPrint(
                          '[DIAGNOSIS_CANVAS] templateConfig hashCode: ${controller.templateConfig.hashCode}',
                        );
                        debugPrint(
                          '[DIAGNOSIS_CANVAS] --------------------------------------------',
                        );
                        ...
                        if (controller.templateConfig.isEmpty) {
                          return Center(
                            child: Text('no_template_data_found'.trFormat),
                          );
                        }
          ...
          Obx(() {
            final _ = controller.templateConfig.values
                .toList(); // trigger rebuild on config change
            if (controller.selectedLayerId.value.isNotEmpty) {
  ...
  Widget _buildInlineSizePanel() {
    ...
    if (!isText) {
      ...
      currentSize = toD(
        layer['fontSize'] ?? layer['font_size'] ?? layer['size'],
        48.0,
      );
      minSize = 8.0;
      maxSize = 400.0;
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            !isText ? Icons.photo_size_select_small : Icons.text_fields,
            size: 16,
          ),
          Expanded(
            child: Slider(
              ...
              onChanged: (val) {
                ...
                if (!isText) {
                  ...
                  final double aspect = oldScaleX > 0
                      ? (oldScaleY / oldScaleX)
                      : 1.0;
                  ...
                  final String textKind = (layer['kind'] ?? '')
                      .toString()
                      .toLowerCase();
                  final double oldW = (layer['w'] ?? layer['width'] ?? 0)
                      .toDouble();
          ...
          Icon(
            !isText ? Icons.photo_size_select_large : Icons.text_fields,
            size: 24,
          ),
        ],
      ),
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final activeColor =
              (layer['font_color'] ?? layer['tint_color'] ?? layer['color'])
                  ?.toString()
                  .toUpperCase() ??
              '';
          ...
          return GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                shape: BoxShape.circle,
                border: Border.all(
                  color: isSelected ? const Color(0xFF5538EE) : Colors.black12,
  ...
  Widget _buildContextualToolbar() {
    ...
    final bool isText =
        ...
        (activeLayer['type'] == 'text' ||
            activeLayer['type'] == 'i-text' ||
            activeLayer['text'] != null);
    final bool isShape =
        activeLayer != null &&
        !isText &&
        (activeLayer['is_shape'] == true ||
            activeLayer['is_shape'] == 'true' ||
            activeLayer['is_shape'] == 1 ||
    ...
    final bool isImage =
        ...
        !isText &&
        !isShape &&
        (activeLayer['type'] == 'image' || activeLayer['src'] != null);
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
              ...
              child: Row(
                ...
                children: [
                    ...
                    _buildToolBtn(Icons.edit_outlined, 'Edit', () {
                      ...
                      if (layer != null &&
                          (layer['type'] == 'text' ||
                              ...
                              layer['text'] != null)) {
                        controller.activeTool.value =
                            controller.activeTool.value == 'Edit' ? '' : 'Edit';
                      }
                    }),
                  _buildToolBtn(Icons.open_with, 'nudge'.trFormat, () {
                    ...
                    if (layer != null) {
                      controller.activeTool.value =
                          controller.activeTool.value == 'Nudge' ? '' : 'Nudge';
                    }
                  }),
                  if (isText)
                    _buildToolBtn(Icons.text_fields, 'font'.trFormat, () {
                      final layer = _getActiveLayer();
                      if (layer != null &&
                          (layer['type'] == 'text' ||
                              layer['type'] == 'i-text' ||
                              layer['text'] != null)) {
                        controller.activeTool.value =
                            controller.activeTool.value == 'Font' ? '' : 'Font';
                      }
                    }),
                  if (isText)
                    _buildToolBtn(Icons.format_size, 'size'.trFormat, () {
                      final layer = _getActiveLayer();
                      if (layer != null &&
                          (layer['type'] == 'text' ||
                              layer['type'] == 'i-text' ||
                              layer['text'] != null)) {
                        controller.activeTool.value =
                            controller.activeTool.value == 'Size' ? '' : 'Size';
                      }
                    }),
                  if (isText)
                    _buildToolBtn(
                  ...
                  if (isText || isShape)
                    _buildToolBtn(Icons.palette_outlined, 'color'.trFormat, () {
                      final layer = _getActiveLayer();
                      if (layer != null) {
                        controller.activeTool.value =
                            controller.activeTool.value == 'Color'
                            ? ''
                            : 'Color';
                      }
                    }),
                  ...
                  if (isImage || isShape)
                    _buildToolBtn(Icons.flip, 'mirror'.trFormat, () {
                      final layer = _getActiveLayer();
                      if (layer != null) {
                        bool currentFlipX =
                            layer['flipX'] == true || layer['flipX'] == 'true';
                        controller.updateLayerProperties(
                          controller.selectedLayerId.value,
                          {'flipX': !currentFlipX},
                        );
                      }
            ...
            Obx(() {
              if (controller.activeTool.value == 'Edit')
                return _buildInlineEditPanel();
              if (controller.activeTool.value == 'Nudge')
                return _buildInlineNudgePanel();
              if (controller.activeTool.value == 'Font')
                return _buildInlineFontPanel();
              if (controller.activeTool.value == 'Size')
                return _buildInlineSizePanel();
              if (controller.activeTool.value == 'Color')
                return _buildInlineColorPanel();
              return const SizedBox.shrink();
            }),
            Obx(
              () => SizedBox(
                height: controller.activeTool.value.isNotEmpty ? 16 : 0,
              ),
            ),
          ],
        ),
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  children: [
                    _buildFieldBadge(
                      'name_label'.trFormat,
                      '_b_name,name,business_name',
                    ),
                    _buildFieldBadge('logo_label'.trFormat, '_b_logo,logo'),
                    _buildIconBadge(
                      Icons.phone_android,
                      '_b_phone,phone_icon,mobile_icon,call_icon',
                    ),
                    _buildFieldBadge(
                      'mobile_label'.trFormat,
                      'mobile,phone,phone_text,mobile_text',
                    ),
                    _buildIconBadge(Icons.mail_outline, '_b_email,email_icon'),
                    _buildFieldBadge(
                      'email_label'.trFormat,
                      'email,email_text',
                    ),
                    _buildIconBadge(
                      Icons.location_on_outlined,
                      '_b_address,address_icon,location_icon',
                    ),
                    _buildFieldBadge(
                      'address_label'.trFormat,
                      'address,location,address_text,location_text',
                    ),
                    _buildIconBadge(
                      Icons.language,
                      '_b_website,web_icon,website_icon',
                    ),
                    _buildFieldBadge(
                      'website_label'.trFormat,
                      'web,website,web_text,website_text',
                    ),
                    _buildFieldBadge(
                      'frame_label'.trFormat,
                      '_frame_bg,_frame,frame,Frame_Bg,bg_frame',
                    ),
                  ],
                ),
              ),
                ...
                return SizedBox(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final thumbUrl =
                          thumbBaseUrl.isNotEmpty && updatedAt.isNotEmpty
                      ...
                      return GestureDetector(
                        onTap: () async {
                          final int tapTimeMs =
                              DateTime.now().millisecondsSinceEpoch;
                          final String prevFrameId =
                              (controller.templateConfig['id'] ?? '')
                                  .toString();
                          final String selectedTemplate =
                              (controller.templateConfig['info']?['id'] ??
                                      controller.templateConfig['zip_name'] ??
                                      '')
                                  .toString();
                          ...
                          final curLayers =
                              controller.templateConfig['layers']
                                  as List<dynamic>? ??
                              [];
                          final frameLayerIds = curLayers
                              .where(
                                (l) =>
                                    l['_is_frame_layer'] == true ||
                                    l['_isFrameLayer'] == true,
                              )
                              .map((l) => (l['id'] ?? l['name']).toString())
                              .toList();
                          ...
                          NativeEditorController.timelineInteractiveLayerBuild =
                              null;
                          NativeEditorController.timelinePlaceholderCalled =
                              false;
                          NativeEditorController.timelineImageBuilderCalled =
                              false;
                          ...
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] Timestamp: $tapTimeMs',
                          );
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] Selected Frame ID: $frameId',
                          );
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] Selected Frame Name: $frameName',
                          );
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] Current Template Layer Count: ${curLayers.length}',
                          );
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] Current Frame Layer IDs: $frameLayerIds',
                          );
                          debugPrint(
                            '[DIAGNOSIS_CANVAS] ------------------------------------------------',
                          );
                          ...
                          if (jsonStr != null) {
                            try {
                              ...
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] Calling loadNewFrame()',
                              );
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] Timestamp: ${DateTime.now().millisecondsSinceEpoch}',
                              );
                              await controller.loadNewFrame(config);
                              final int afterLoadTime =
                                  DateTime.now().millisecondsSinceEpoch;
                              final postLayers =
                                  controller.templateConfig['layers']
                                      as List<dynamic>? ??
                                  [];
                              final postLayerIds = postLayers
                                  .map((l) => (l['id'] ?? l['name']).toString())
                                  .toList();
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] loadNewFrame Finished',
                              );
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] Timestamp: $afterLoadTime',
                              );
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] Current templateConfig layer count: ${postLayers.length}',
                              );
                              debugPrint(
                                '[DIAGNOSIS_CANVAS] Current layer IDs: $postLayerIds',
                              );
                            } catch (e) {
                            ...
                            try {
                              final Map<String, dynamic> config =
                                  configObj is String
                                  ? jsonDecode(configObj)
                                  : Map<String, dynamic>.from(configObj);
                              // Resolve relative src paths using full_url as base
                              final String fullUrl = (frame['full_url'] ?? '')
                                  .toString();
                              ...
                              if (fullUrl.isNotEmpty &&
                                  config['layers'] != null) {
                                final skinBase = fullUrl.substring(
                                  0,
                                  fullUrl.lastIndexOf('/') + 1,
                                );
                                final tplBase = skinBase.replaceAll(
                                  RegExp(r'skins/[^/]+/$'),
                                  '',
                                );
                                for (var layer in (config['layers'] as List)) {
                                  for (String urlField in [
                                    'src',
                                    '_fallback_src',
                                  ]) {
                                    if (layer[urlField] != null) {
            ...
            children: [
              ...
              Expanded(
                child: Obx(() {
                  ...
                  final allLayers = List<dynamic>.from(
                    controller.templateConfig['layers'] ?? [],
                  );
                  final frameLayersCount = allLayers
                      .where(
                        (l) =>
                            l['_is_frame_layer'] == true ||
                            l['_isFrameLayer'] == true,
                    ...
                    child: ReorderableListView.builder(
                      ...
                      onReorder: (oldIndex, newIndex) {
                        ...
                        if (targetCustomIndex >= remainingCustomLayers.length) {
                          if (remainingCustomLayers.isEmpty) {
                            ...
                            actualNew =
                                newAllLayers.indexOf(
                                  remainingCustomLayers.last,
                                ) +
                                1;
                          }
                          ...
                          if (remainingCustomLayers.isEmpty) {
                            ...
                            actualNew = newAllLayers.indexOf(
                              remainingCustomLayers.first,
                            );
                          }
                        } else {
                          actualNew = newAllLayers.indexOf(
                            remainingCustomLayers[targetCustomIndex],
                          );
                        }
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final isText =
                            ...
                            layer['text'] != null;
                        final titleStr =
                            (layer['name'] ?? layer['id'] ?? 'Unnamed Layer')
                                .toString()
                                .replaceAll(RegExp(r'[-_]'), ' ')
                                .toUpperCase();
                        debugPrint(
                          '[LAYERS_MODAL] Layer: name=${layer['name']} type=${layer['type']} src=${(layer['src'] ?? '').toString().substring(0, (layer['src'] ?? '').toString().length > 80 ? 80 : (layer['src'] ?? '').toString().length)} iconName=${layer['iconName']} _is_frame=${layer['_is_frame_layer']}',
                        );
                        Widget previewWidget;
                        if (isText) {
                          String txt = (layer['text']?.toString() ?? 'T')
                              .trim();
                          ...
                          if (src.startsWith('data:')) {
                            ...
                            previewWidget = ClipRRect(
                              ...
                              (layer['iconName'] != null &&
                                  layer['iconName'].toString().isNotEmpty)) {
                            previewWidget = Container(
                              width: 40,
                              height: 40,
                              decoration: BoxDecoration(
  ...
  void _showAiTextModal(BuildContext context) {
    ...
    final String catName = (widget.frameData['customCategoryName'] ?? '')
        .toString()
        .toLowerCase();
    ...
    _aiCurrentStep.value = isHiring
        ? 'hiring_questionnaire'
        : 'menu'; // Reset on open
    ...
    for (var layer in layers) {
      if (layer['type'] == 'image' && layer['is_background'] != true) {
        final name = (layer['name'] ?? layer['id'] ?? '')
            .toString()
            .toLowerCase();
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton(
                            ...
                            onPressed: () => _aiCurrentStep.value = isHiring
                                ? 'hiring_questionnaire'
                                : 'menu',
                        ...
                        Expanded(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              ...
                              Get.snackbar(
                                'success'.trFormat,
                                'ai_text_applied'.trFormat,
                                backgroundColor: Colors.green,
                                colorText: Colors.white,
                              );
                            },
                            child: Text(
                              'apply_to_design'.trFormat,
                              style: const TextStyle(
                                color: Colors.white,
                                fontWeight: FontWeight.bold,
                              ),
                                  ...
                                  ? Row(
                                      children: [
                                        ClipRRect(
                                          ...
                                          child:
                                              (_aiSelectedProduct!['_display_image'] ??
                                                      '')
                                                  .toString()
                                                  .isNotEmpty
                                              ? Image.network(
                                        ...
                                        Expanded(
                                          child: Column(
                                            ...
                                            children: [
                                              Text(
                                                _aiSelectedProduct!['_display_name'] ??
                                                    _aiSelectedProduct!['title'] ??
                                                    _aiSelectedProduct!['name'] ??
                                                    'Selected Product',
                                                style: const TextStyle(
                                                  fontWeight: FontWeight.bold,
                                                  fontSize: 16,
                                              ...
                                              if ((_aiSelectedProduct!['_display_category'] ??
                                                      _aiSelectedProduct!['category_name'] ??
                                                      '')
                                                  .toString()
                                                  .isNotEmpty)
                                                Text(
                                                  _aiSelectedProduct!['_display_category'] ??
                                                      _aiSelectedProduct!['category_name'] ??
                                                      '',
                                                  style: const TextStyle(
                                                    color: Colors.grey,
                                              ...
                                              if ((_aiSelectedProduct!['_display_price'] ??
                                                      _aiSelectedProduct!['price'] ??
                                                      '')
                                                  .toString()
                                                  .isNotEmpty)
                                                Text(
                                                  '\u20B9${_aiSelectedProduct!['_display_price'] ?? _aiSelectedProduct!['price']}',
                                                  style: const TextStyle(
                                                    color: Colors.green,
                                                    fontWeight: FontWeight.w600,
                                                    fontSize: 14,
                      ...
                      _buildAiOptionTile(
                        ...
                        onTap: () {
                          if (_aiSelectedProduct == null) {
                            Get.snackbar(
                              'product_required'.trFormat,
                              'please_select_product_first'.trFormat,
                            );
                            return;
                          }
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text(
                            'ai_hiring_post_generator'.trFormat,
                            style: const TextStyle(
                              fontSize: 18,
                              fontWeight: FontWeight.bold,
                            ),
                      ...
                      const SizedBox(height: 16),
                      const Text(
                        'आप किस Role/Post के लिए हायर कर रहे हैं?',
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w600,
                          color: Colors.black87,
                      ...
                      TextField(
                        ...
                        decoration: InputDecoration(
                          hintText:
                              'e.g., Graphic Designer, Sales Executive...',
                          border: OutlineInputBorder(
                            borderRadius: BorderRadius.circular(12),
                          ),
                          contentPadding: const EdgeInsets.symmetric(
                      ...
                      SizedBox(
                        ...
                        child: ElevatedButton(
                          ...
                          onPressed: isGenerating.value
                              ? null
                              : () async {
                                  if (_aiHiringRoleController.text
                                      .trim()
                                      .isEmpty) {
                                    Get.snackbar(
                                      'error'.trFormat,
                                      'please_enter_role_post'.trFormat,
                                    );
                                    return;
                                  }
                                  ...
                                  String contextPrompt =
                                      "Generate a concise and catchy hiring post text for the role of '${_aiHiringRoleController.text.trim()}'.";
                                  if (askRequirements &&
                                      _aiHiringReqController.text
                                          .trim()
                                          .isNotEmpty) {
                                    contextPrompt +=
                                        " Important requirements/salary: '${_aiHiringReqController.text.trim()}'.";
                                  }
                                  ...
                                  if (existingTexts.isNotEmpty) {
                                    contextPrompt +=
                                        " The text must seamlessly replace or complement this existing template content: ${existingTexts.join(', ')}. Keep it short enough to fit the design constraints.";
                                  }
                                  ...
                                  if (content != null && content.isNotEmpty) {
                                    ...
                                    Get.snackbar(
                                      'notice'.trFormat,
                                      'could_not_generate_text'.trFormat,
              ...
              return SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text(
                      'describe_ai_instructions'.trFormat,
                      style: const TextStyle(
                        fontSize: 13,
                        color: Colors.black54,
                      ),
                    ...
                    TextField(
                      ...
                      decoration: InputDecoration(
                        hintText:
                            'E.g., Write a promotional text for a new coffee shop...',
                        border: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(12),
                        ),
                        ...
                        onPressed: isGenerating.value
                            ...
                            : () async {
                                if (_aiPromptController.text.trim().isEmpty) {
                                  Get.snackbar(
                                    'error'.trFormat,
                                    'please_enter_prompt_first'.trFormat,
                                  );
                                  return;
                                }
                                isGenerating.value = true;
  ...
  void _performTier1Generation() {
    ...
    final pName =
        (product['_display_name'] ?? product['title'] ?? product['name'] ?? '')
            .toString();
    final pDesc = (product['description'] ?? product['short_description'] ?? '')
        .toString();
    final pPrice = (product['_display_price'] ?? product['price'] ?? '')
        .toString();
    final imgUrl =
        (product['_display_image'] ??
                product['image_url'] ??
                product['processed_url'] ??
                product['image'] ??
                '')
            .toString();
    final fullUrl = imgUrl.startsWith('http')
        ? imgUrl
        : (imgUrl.isNotEmpty ? '${controller.uploadsBaseUrl}/$imgUrl' : '');
    ...
    for (var layer in layers) {
      ...
      if (layer['type'] == 'text') {
        ...
        if ((lName == 'image1' || lName.contains('product')) &&
    ...
    controller.templateConfig.refresh();
    Get.snackbar(
      'smart_generate'.trFormat,
      'product_details_applied'.trFormat,
      backgroundColor: Colors.green,
      colorText: Colors.white,
    );
  }
  ...
  Future<void> _performTier2Generation(RxBool isGenerating) async {
    ...
    final pDesc = (product['description'] ?? product['short_description'] ?? '')
        .toString();
    final autoPrompt =
        'Write compelling marketing text for this product that perfectly aligns with the template\'s purpose (e.g. sale, hiring, festival, etc). Highlight the key features.';
    ...
    if (content != null && content.isNotEmpty) {
      ...
      Get.snackbar(
        'notice'.trFormat,
        'could_not_generate_enhanced_text'.trFormat,
        backgroundColor: Colors.orange,
        colorText: Colors.white,
      );
    }
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'choose_product_for_ai'.trFormat,
                            style: const TextStyle(
                              fontSize: 12,
                              color: Colors.grey,
                            ),
              ...
              Expanded(
                child: FutureBuilder<http.Response>(
                  ...
                  builder: (context, snapshot) {
                      ...
                      return Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 16),
                            Text(
                              'loading_your_products'.trFormat,
                    ...
                    if (!snapshot.hasData)
                      return Center(
                        child: Text('failed_to_load_products'.trFormat),
                      );
                    try {
                      final body = jsonDecode(snapshot.data!.body);
                      // Debug: print actual API response keys
                      debugPrint(
                        '[ProductPickerAI] API keys: ${body.keys.toList()}',
                      );
                      ...
                      debugPrint(
                        '[ProductPickerAI] Found ${products.length} products',
                      );
                      if (products.isNotEmpty) {
                        debugPrint(
                          '[ProductPickerAI] First product keys: ${(products.first as Map).keys.toList()}',
                        );
                        debugPrint(
                          '[ProductPickerAI] First product: ${products.first}',
                        );
                      }
                        ...
                        return Center(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'add_products_from_business'.trFormat,
                                style: const TextStyle(
                                  color: Colors.grey,
                                  fontSize: 12,
                                ),
                        ...
                        itemBuilder: (context, index) {
                          ...
                          final imgUrl =
                              (product['image_url'] ??
                                      ...
                                      '')
                                  .toString();
                          final fullUrl = imgUrl.isEmpty
                              ? ''
                              : (imgUrl.startsWith('http')
                                    ? imgUrl
                                    : '${controller.uploadsBaseUrl}/$imgUrl');
              ...
              Expanded(
                child: FutureBuilder<http.Response>(
                  ...
                  builder: (context, snapshot) {
                    ...
                    if (!snapshot.hasData)
                      return Center(child: Text('failed'.trFormat));
                    try {
                      final body = jsonDecode(snapshot.data!.body);
                      final List products =
                          body['data'] ?? body['products']?['data'] ?? [];
                      if (products.isEmpty)
                        return Center(
                          child: Text('no_products_found'.trFormat),
                        );
                      return ListView.builder(
                        itemCount: products.length,
                        itemBuilder: (context, index) {
  ...
  Widget _buildFieldBadge(
    ...
    return Obx(() {
      ...
      final _ =
          controller.templateConfig.length; // Ensure GetX tracks this scope
          ...
          content: Column(
            ...
            children: [
              TextField(
                ...
                autofocus: true,
                decoration: InputDecoration(
                  hintText: 'enter_text_here'.trFormat,
                ),
              ),
  ...
  Widget _buildEditingToolsBar() {
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          if (isCustom)
            _buildToolBtn(Icons.image, 'logo'.trFormat, _pickAndAddImage),
          ...
          children: [
            Container(
              ...
              child: Icon(
                ...
                color: isSelected
                    ? Colors.white
                    : (iconColor ?? const Color(0xFF5538EE)),
                size: 24,
              ),
            ),
                    ...
                    try {
                      ...
                      return GridView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final product = products[index];
                          final imgUrl =
                              product['processed_url'] ?? product['image'];
                      ...
                      return Center(
                        child: Text('error_loading_products'.trFormat),
                      );
                    }
                  },
                ),
                  ...
                  Expanded(
                    child: FutureBuilder<http.Response>(
                      future: searchQuery.isEmpty
                          ? http.get(
                              Uri.parse('${AppConfig.baseUrl}/get-sticker'),
                            )
                          : http.post(
                              Uri.parse('${AppConfig.baseUrl}/search-sticker'),
                      ...
                      builder: (context, snapshot) {
                        ...
                        if (snapshot.hasError || !snapshot.hasData) {
                          return Center(
                            child: Text('failed_to_load_stickers'.trFormat),
                          );
                        }
                        try {
                          final body = jsonDecode(snapshot.data!.body);
                          ...
                          } else if (body['stickers'] != null &&
                              body['stickers'] is List) {
                            stickers = body['stickers'];
                          } else if (body is List) {
                            stickers = body;
                          } else if (body['data'] != null &&
                              body['data'] is Map &&
                              body['data']['StickerCategory'] != null) {
                            // Native app get-sticker format
                            for (var category
                                in body['data']['StickerCategory']) {
                              if (category['sticker'] != null) {
                                stickers.addAll(category['sticker']);
                              }
                            }
                          ...
                          if (stickers.isEmpty) {
                            return Center(
                              child: Text('no_stickers_found'.trFormat),
                            );
                          }
                          return GridView.builder(
                            gridDelegate:
                          ...
                          return Center(
                            child: Text('error_parsing_stickers'.trFormat),
                          );
                        }
                      },
                    ),

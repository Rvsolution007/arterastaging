...
class _EditorCanvasWidgetState extends State<EditorCanvasWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    int renderVersion = (widget.config['render_version'] ?? 1) as int;
    renderVersion = 6;
    widget.config['render_version'] = 6;
    // ══ DIAGNOSTICS ══
    debugPrint('╔══════════════════════════════════════════════');
  ...
  Widget _buildLayer(Map<String, dynamic> layer, double scale, int renderVersion) {
    ...
    final dynamic rvRaw = widget.config['render_version'];
    final int rv = rvRaw is int ? rvRaw : int.tryParse(rvRaw?.toString() ?? '1') ?? 1;
    debugPrint('[V6_ROUTER] text layer "${layer['name'] ?? layer['id']}", render_version=$rv');
    if (rv >= 6) return _buildTextV6(layer, scale);
    return _buildTextLegacy(layer, scale);
  }

Widget _buildTextLegacy(Map<String, dynamic> layer, double scale) {
  ...
  Widget _buildTextV6(Map<String, dynamic> layer, double scale) {
    final String layerName = (layer['name'] ?? layer['id'] ?? '').toString();
    String textValue = layer['text']?.toString() ?? '';
    ...
    // V6 DIAGNOSTICS
    debugPrint('\n[V6_DIAG] --- TEXT LAYER: $layerName ---');
    debugPrint('[V6_DIAG] render_version: ${widget.config['render_version']}');
    debugPrint('[V6_DIAG] text: "$textValue"');
    debugPrint('[V6_DIAG] rawW: $rawW, isSingleLine: $isSingleLine');
    debugPrint('[V6_DIAG] colorStr: $colorStr, fontColor: $fontColor');
    debugPrint('[V6_DIAG] paragraphWidth in layer: ${layer['paragraphWidth']}');
    debugPrint('[V6_DIAG] original_color in layer: ${layer['original_color']}');
    
    if (rawW > 0 && !isSingleLine) {
      final double paragraphW = safeDouble(layer['paragraphWidth'] ?? layer['w'] ?? layer['width'] ?? 0);
      debugPrint('[V6_DIAG] Computed paragraphW: $paragraphW');
      if (paragraphW > 0) {
        textWidget = SizedBox(width: paragraphW * scale, child: textWidget);
      } else {
        textWidget = Container(width: double.infinity, alignment: alignment, child: textWidget);
      }
    } else if (isSingleLine && rawW > 0) {
      // Point text (single-line) needs a fixed-width container so alignment (right/center/left)
      // takes effect, and FittedBox(scaleDown) ensures long text (like email) shrinks without overlapping.

...
class _EditorCanvasWidgetState extends State<EditorCanvasWidget> {
  void _logLifecycle(String methodName) {
    ...
    final String templateId =
        (widget.config['info']?['id'] ?? widget.config['zip_name'] ?? '')
            .toString();
    debugPrint(
      '[DIAGNOSIS_CANVAS] ========================================================',
    );
  ...
  void _captureNativeGolden(
    ...
    for (var layer in layers) {
      ...
      if (layer['type'] == 'text') {
        final double rawSize = safeDouble(
          layer['fontSize'] ?? layer['font_size'] ?? layer['size'] ?? 16,
        );
        final double ppiScale =
            safeDouble(widget.config['info']?['ppi'] ?? 72) / 72.0;
        final double layerScaleY = safeDouble(layer['scaleY'] ?? 1);
        nativeComputed[name]!['finalFontSize'] =
            (rawSize * ppiScale * layerScaleY * scale).roundToDouble();
      }
    }
    ...
    final zipName =
        widget.config['info']?['zip_name'] ?? widget.config['zip_name'] ?? '';
  ...
  void _fetchLogoDimensions() {
    ...
    final mappings =
        (widget.aiData != null && widget.aiData!['_image_mappings'] != null)
    ...
    for (var layer in layers) {
      final String name = (layer['name'] ?? layer['id'] ?? '')
          ...
          .toLowerCase();
      final bool isLogo =
          layer['_businessKey'] == 'logo' || name.contains('logo');
      if (!isLogo) continue;
      ...
      if (mappings[name] != null) {
        ...
        for (var key in mappings.keys) {
          if (cleanLName ==
              key.replaceAll(RegExp(r'[\s\-_]'), '').toLowerCase()) {
      ...
      if (mappedUrl == null &&
          (cleanLName == 'image1' || cleanLName == 'mainimage')) {
        mappedUrl =
            mappings['image1'] ?? mappings['main_image'] ?? mappings['image 1'];
      }
          ...
          .addListener(
            ImageStreamListener(
              ...
              onError: (dynamic exception, StackTrace? stackTrace) {
                debugPrint(
                  '⚠️ Error resolving logo dimensions for $layerId: $exception',
                );
              },
            ),
          );
  ...
  void didUpdateWidget(EditorCanvasWidget oldWidget) {
    ...
    final bool templateChanged =
        oldWidget.config['info']?['width'] != widget.config['info']?['width'] ||
        (oldWidget.config['layers'] as List?)?.length !=
            (widget.config['layers'] as List?)?.length;
    ...
    final String oldTemplateId =
        (oldWidget.config['info']?['id'] ?? oldWidget.config['id'] ?? '')
            .toString();
    final String newTemplateId =
        (widget.config['info']?['id'] ?? widget.config['id'] ?? '').toString();
    ...
    final int oldLayersLength =
        (oldWidget.config['layers'] as List?)?.length ?? 0;
    ...
    final double oldCanvasHeight =
        oldWidget.config['info']?['height']?.toDouble() ?? 0.0;
    final double newCanvasHeight =
        widget.config['info']?['height']?.toDouble() ?? 0.0;
    ...
    final oldLayerIds = ((oldWidget.config['layers'] as List?) ?? [])
        .map((l) => (l['id'] ?? l['name']).toString())
        .toList();
    final newLayerIds = ((widget.config['layers'] as List?) ?? [])
        .map((l) => (l['id'] ?? l['name']).toString())
        .toList();
    ...
    for (final layer in _newLayersList) {
      debugPrint(
        '[DIAGNOSIS_CANVAS] ${layer['id']} - ${layer['type']} - ${layer['name']}',
      );
    }
  ...
  void _measureAndApplyShifts() {
    final double designW = safeDouble(
      widget.config['info']?['width'] ?? widget.config['width'] ?? 1080,
    );
    final double designH = safeDouble(
      widget.config['info']?['height'] ?? widget.config['height'] ?? 1080,
    );
    final double scale = widget.width / designW;
    final List<dynamic> layers = widget.config['layers'] ?? [];
    final double maxBottom =
        designH - 30; // 30px padding at bottom (matches web editor)
    ...
    for (var rawLayer in layers) {
      ...
      final double currentH =
          renderBox.size.height / scale; // convert to design pixels
      ...
      if (!_origHeights.containsKey(name)) {
        ...
        debugPrint(
          '[BASELINE] "$name": _origHeight=${currentH.toStringAsFixed(1)}',
        );
      ...
      debugPrint(
        '[FLEX] "$name": baselineH=${baselineH.toStringAsFixed(1)} '
    ...
    for (var src in textSources) {
      ...
      for (var rawLayer in layers) {
        ...
        final bool overlapsX =
            (objLeft < src.origRight + 20) && (objRight > src.origLeft - 20);
  ...
  double _measureTextHeight(Map<String, dynamic> layer, double scale) {
    ...
    final double rawFontSize = safeDouble(
      layer['fontSize'] ?? layer['font_size'] ?? layer['size'] ?? 16,
    );
    ...
    final double lineHeight =
        safeDouble(layer['lineHeight'] ?? layer['line_height']) ?? 1.16;
    final double layerW = safeDouble(
      layer['w'] ?? layer['width'] ?? 0,
    ); // design px
    ...
    final double avgCharWidth = fontSize * avgCharWidthRatio + extraCharWidth;
  ...
  Widget build(BuildContext context) {
    ...
    final String frameId = (widget.config['id'] ?? '').toString();
    final String templateId =
        (widget.config['info']?['id'] ??
                widget.config['id'] ??
                widget.config['zip_name'] ??
                '')
            .toString();
    ...
    debugPrint('╔══════════════════════════════════════════════');
    debugPrint(
      '║ EditorCanvasWidget: ${layers.length} layers, '
    ...
    sortedLayers.sort((a, b) {
      final int zA = (a['z_index'] ?? 0) is int
          ? (a['z_index'] ?? 0)
          : ((a['z_index'] ?? 0) as num).toInt();
      final int zB = (b['z_index'] ?? 0) is int
          ? (b['z_index'] ?? 0)
          : ((b['z_index'] ?? 0) as num).toInt();
    ...
    for (int i = 0; i < sortedLayers.length; i++) {
      ...
      debugPrint(
        '  [LAYER $i] name="${l['name']}" type="${l['type']}" originalType="${l['_originalType']}" iconName="${l['iconName']}" src="${l['src'] != null ? (l['src'].toString().length > 100 ? l['src'].toString().substring(0, 100) + '...' : l['src']) : 'null'}" _source_meta=${l['_source_meta']} tint_color=${l['tint_color']} font_color=${l['font_color']} color=${l['color']}',
      );
    }
    ...
    if (_measuredShifts.isNotEmpty) {
      for (var layer in adjusted) {
        ...
        if (_measuredShifts.containsKey(name)) {
          layer['y'] = safeDouble(layer['y'] as num) + _measuredShifts[name]!;
        }
      }
    }
    ...
    for (int idx = 0; idx < adjusted.length; idx++) {
      ...
      if (ltype == 'image' && layer['mask_layer_id'] == null && !isShape) {
        ...
        debugPrint(
          '[MASK_PREPASS] Checking "${layer['name']}" (idx=$idx) pos=($myX,$myY,$myW,$myH)',
        );

        for (int i = idx - 1; i >= 0; i--) {
          final candidate = adjusted[i];
          final bool candIsShape =
              candidate['is_shape'] == true || candidate['is_shape'] == 1;
          ...
          final double cw = safeDouble(
            candidate['w'] ?? candidate['width'] ?? 0,
          );
          final double ch = safeDouble(
            candidate['h'] ?? candidate['height'] ?? 0,
          );
          ...
          if ((myX - cx).abs() < 2 &&
              ...
              (myH - ch).abs() < 2) {
            debugPrint(
              '[MASK_PREPASS] ✅ MATCH! "${layer['name']}" clips to "${candidate['name']}"',
            );
            layer['mask_layer_id'] = candidate['name'];
            // Mark the mask shape so _buildLayer hides it
            candidate['_is_used_as_mask'] = true;
            debugPrint(
              '[MASK_PREPASS] 🔒 Marked "${candidate['name']}" as _is_used_as_mask=true',
            );
            break;
          }
        }
    ...
    for (int idx = 0; idx < adjusted.length; idx++) {
      ...
      if (isLogo && _logoDimensions.containsKey(layerId)) {
        ...
        if (rawW > 0 && rawH > 0 && trueSize.width > 0 && trueSize.height > 0) {
          ...
          final double canvasHeight = safeDouble(
            widget.config['height'] ?? 1080,
          );
    ...
    debugPrint(
      '[DIAGNOSIS_CANVAS] Current Widget Tree: EditorCanvas -> Container -> Stack',
    );
    ...
    return Container(
      ...
      child: Stack(clipBehavior: Clip.hardEdge, children: stackChildren),
    );
  }
  ...
  Widget _buildLayer(
    ...
    final String rawName = (layer['name'] ?? layer['id'] ?? '')
        .toString(); // original case for key lookup
    ...
    debugPrint(
      '🔴 [DIAGNOSIS-LAYER] name="$name" type="$type" isShape=${layer['is_shape']} isFrameLayer=$isFrameLayer isBackground=$isBackground',
    );
    ...
    if (!isFrameLayer &&
      ...
      debugPrint(
        '[LAYER_HIDE] Hiding "$name" — is_background explicitly false',
      );
      debugPrint(
        '[DIAGNOSIS_CANVAS] --------------------------------------------',
      );
      ...
      debugPrint(
        '[DIAGNOSIS_CANVAS] Reason: Hiding "$name" — is_background explicitly false',
      );
    ...
    final bool isUsedAsMask =
        layer['_is_used_as_mask'] == true ||
        allLayers.any((otherLayer) {
          final otherMaskId = otherLayer['mask_layer_id'];
          if (otherMaskId == null) return false;
          final maskIdStr = otherMaskId.toString();
          return maskIdStr == rawName ||
              maskIdStr == (layer['id']?.toString() ?? '');
        });
    if (isUsedAsMask) {
      debugPrint(
        '[LAYER_HIDE] Hiding "$name" — used as mask shape by another layer',
      );
      debugPrint(
        '[DIAGNOSIS_CANVAS] --------------------------------------------',
      );
      ...
      debugPrint('[DIAGNOSIS_CANVAS] RuntimeType: SizedBox');
      debugPrint(
        '[DIAGNOSIS_CANVAS] Reason: Hiding "$name" — used as mask shape by another layer',
      );
      debugPrint('[DIAGNOSIS_CANVAS] Layer Count: ${allLayers.length}');
      debugPrint(
        '[DIAGNOSIS_CANVAS] --------------------------------------------',
    ...
    if ((layer['is_shape'] == true || layer['is_shape'] == 1) &&
        layer['tint_color'] != null) {
      debugPrint(
        '[LAYER_DIAG] Processing decorated shape/icon (has tint_color): "$name"',
      );
    }
    ...
    debugPrint(
      '║   is_background_raw=${layer['is_background']} (${layer['is_background'].runtimeType})',
    );
    ...
    debugPrint(
      '║   native=${rawW}x$rawH scaled=${(rawW * scale).toStringAsFixed(1)}x${(rawH * scale).toStringAsFixed(1)}',
    );
    ...
    if (type == 'image' && layer['mask_layer_id'] != null) {
      ...
      final maskShapeLayer = allLayers.firstWhere(
        (l) =>
            (l['name']?.toString() == maskId) ||
            (l['id']?.toString() == maskId),
      ...
      if (maskShapeLayer.isNotEmpty) {
        ...
        debugPrint(
          '[MASK_FIX] "$name" position overridden to mask shape "${maskShapeLayer['name']}": '
        ...
        debugPrint(
          '[MASK_FIX] ⚠️ "$name" mask_layer_id="$maskId" but NO matching layer found!',
        );
      }
    }
    ...
    final double nativeW = safeDouble(
      effectiveLayer['w'] ?? effectiveLayer['width'] ?? 0,
    );
    final double nativeH = safeDouble(
      effectiveLayer['h'] ?? effectiveLayer['height'] ?? 0,
    );
    ...
    if (type == 'text') {
      ...
      final double rawSize = safeDouble(
        effectiveLayer['fontSize'] ??
            effectiveLayer['font_size'] ??
            effectiveLayer['size'] ??
            16,
      );
      final double docPPI = safeDouble(widget.config['info']?['ppi'] ?? 72);
      ...
      final String textKind = (effectiveLayer['kind'] ?? '')
          .toString()
          .toLowerCase();
      ...
      debugPrint(
        '│   json.size=${layer['size']}  json.font_size=${layer['font_size']}  json.fontSize=${layer['fontSize']}',
      );
      debugPrint(
        '│   json.kind=${layer['kind']}  json.weight=${layer['weight']}  json.style=${layer['style']}',
      );
      debugPrint(
        '│   json.font=${layer['font']}  json.font_name=${layer['font_name']}  json.fontFamily=${layer['fontFamily']}',
      );
      debugPrint(
        '│   json.color=${layer['color']}  json.fill=${layer['fill']}',
      );
      debugPrint(
        '│   json.justification=${layer['justification']}  json.textAlign=${layer['textAlign']}',
      );
      debugPrint(
        '│   json.lineHeight=${layer['lineHeight']}  json.line_height=${layer['line_height']}',
      );
      debugPrint(
        '│   json.letterSpacing=${layer['letterSpacing']}  json.char_spacing=${layer['char_spacing']}',
      );
      debugPrint(
        '│   json.wordSpacing=${layer['wordSpacing']}  json.opacity=${layer['opacity']}',
      );
      debugPrint(
        '│   json.scaleX=${layer['scaleX']}  json.scaleY=${layer['scaleY']}',
      );
      debugPrint(
        '│   json.flipX=${layer['flipX']}  json.flipY=${layer['flipY']}',
      );
      debugPrint('│   ── COMPUTED VALUES ──');
      debugPrint(
        '│   rawSize=$rawSize  docPPI=$docPPI  ppiScale=$ppiScale  fontSize(design)=$fontSize',
      );
      ...
      debugPrint(
        '│   finalX_scaled=${safeDouble(effectiveLayer['x'] ?? 0) * scale}',
      );
      debugPrint(
        '│   finalY_scaled_before_point_fix=${safeDouble(effectiveLayer['y'] ?? 0) * scale}',
      );
      if (textKind == 'point') {
        final double layerScaleYForFont = safeDouble(
          effectiveLayer['scaleY'] ?? effectiveLayer['scaleX'] ?? 1.0,
        );
        final double effectiveFontSizeScaled =
            rawSize * ppiScale * layerScaleYForFont * scale;
        final double pointFix = effectiveFontSizeScaled * 0.12;
        debugPrint(
          '│   POINT_TEXT_FIX: effectiveFontSizeScaled=$effectiveFontSizeScaled  pointFix(0.12)=$pointFix',
        );
        debugPrint(
          '│   finalY_after_all_fixes=${safeDouble(effectiveLayer['y'] ?? 0) * scale - pointFix}',
        );
      } else {
        debugPrint('│   (not point text, no InteractiveLayer point fix)');
        debugPrint(
          '│   finalY_after_all_fixes=${safeDouble(effectiveLayer['y'] ?? 0) * scale}',
        );
      }
      debugPrint(
        '│   finalW_scaled=${safeDouble(effectiveLayer['w'] ?? effectiveLayer['width'] ?? 0) * scale}',
      );
      ...
      int dupeIndex = 1;
      while (_textKeys.containsKey(uniqueKeyName) &&
          _textKeys[uniqueKeyName] != null) {
      ...
      debugPrint(
        '│   json.x=${effectiveLayer['x']}  json.y=${effectiveLayer['y']}',
      );
      debugPrint(
        '│   json.w=${effectiveLayer['w']}  json.h=${effectiveLayer['h']}',
      );
      debugPrint('│   json.src=${effectiveLayer['src']}');
      debugPrint(
        '│   json.opacity=${effectiveLayer['opacity']}  json.scaleX=${effectiveLayer['scaleX']}  json.scaleY=${effectiveLayer['scaleY']}',
      );
      debugPrint(
        '│   json.flipX=${effectiveLayer['flipX']}  json.flipY=${effectiveLayer['flipY']}',
      );
      debugPrint(
        '│   json.is_background=${effectiveLayer['is_background']}  json.is_shape=${effectiveLayer['is_shape']}',
      );
      debugPrint(
        '│   json.is_placeholder=${effectiveLayer['is_placeholder']}  json.is_slot=${effectiveLayer['is_slot']}',
      );
      debugPrint('│   scale=$scale');
      debugPrint(
        '│   finalX=${safeDouble(effectiveLayer['x'] ?? 0) * scale}  finalY=${safeDouble(effectiveLayer['y'] ?? 0) * scale}',
      );
      ...
      final bool _hasIconMeta =
          effectiveLayer['_source_meta'] is Map &&
      ...
      final bool _hasTopIconName =
          effectiveLayer['iconName'] != null &&
          effectiveLayer['iconName'].toString().isNotEmpty;

      if (_hasIconMeta || _hasOrigTypeIcon || _hasTopIconName) {
        debugPrint(
          '[ICON_ROUTE] type=image but detected icon metadata → routing to _buildIconLayer (meta=$_hasIconMeta origType=$_hasOrigTypeIcon topName=$_hasTopIconName)',
        );
        content = _buildIconLayer(
          effectiveLayer,
          name,
          scale,
          nativeW,
      ...
      debugPrint(
        '│   json.fill=${effectiveLayer['fill']}  json.fillColor=${effectiveLayer['fillColor']}',
      );
      debugPrint(
        '│   json.stroke=${effectiveLayer['stroke']}  json.strokeWidth=${effectiveLayer['strokeWidth']}',
      );
      debugPrint(
        '│   json.shapeType=${effectiveLayer['shapeType']}  json.shape_type=${effectiveLayer['shape_type']}',
      );
      debugPrint(
        '│   json.rx=${effectiveLayer['rx']}  json.ry=${effectiveLayer['ry']}',
      );
      debugPrint(
        '│   json.scaleX=${effectiveLayer['scaleX']}  json.scaleY=${effectiveLayer['scaleY']}',
      );
      debugPrint(
        '│   json.opacity=${effectiveLayer['opacity']}  json.rotation=${effectiveLayer['rotation']}',
      );
      debugPrint(
        '│   json.flipX=${effectiveLayer['flipX']}  json.flipY=${effectiveLayer['flipY']}',
      );
      debugPrint(
        '│   json.is_shape=${effectiveLayer['is_shape']}  json.is_background=${effectiveLayer['is_background']}',
      );
      debugPrint(
        '│   json.svgPath=${(effectiveLayer['svgPath'] ?? '').toString().length > 0 ? 'yes(${(effectiveLayer['svgPath'] ?? '').toString().length} chars)' : 'no'}',
      );
      debugPrint(
        '│   json.cornerRadius=${effectiveLayer['cornerRadius']}  json.borderRadius=${effectiveLayer['borderRadius']}',
      );
      debugPrint('│   scale=$scale');
      debugPrint(
        '│   finalX=${safeDouble(effectiveLayer['x'] ?? 0) * scale}  finalY=${safeDouble(effectiveLayer['y'] ?? 0) * scale}',
      ...
      debugPrint(
        '│   json.color=${effectiveLayer['color']}  json.fill=${effectiveLayer['fill']}',
      );
      debugPrint(
        '│   json.tint_color=${effectiveLayer['tint_color']}  json.font_color=${effectiveLayer['font_color']}',
      );
      debugPrint(
        '│   json.scaleX=${effectiveLayer['scaleX']}  json.scaleY=${effectiveLayer['scaleY']}',
      );
  ...
  Widget _buildText(Map<String, dynamic> layer, double scale) {
    ...
    final dynamic rawCharSpacing =
        layer['letterSpacing'] ?? layer['char_spacing'];
  ...
  Color _parseColor(
    ...
    if (hex.length == 8) {
      ...
      debugPrint(
        '[PARSE_COLOR] colorStr="$colorStr" -> hex="$hex" -> parsed=$parsed',
      );
      if (parsed != null) return Color(parsed);
    } else {
      debugPrint(
        '[PARSE_COLOR] Unknown hex format: "$colorStr" -> "$hex" (length ${hex.length})',
      );
    }
  ...
  String _resolveAssetUrl(String src) {
    ...
    if ((baseUrl.isEmpty || !baseUrl.contains('/uploads/template/')) &&
        Get.isRegistered<NativeEditorController>()) {
    ...
    String zipName = (widget.config['zip_name'] ?? widget.config['path'] ?? '')
        .toString()
        .replaceAll('/', '')
        .trim();
    if (zipName.isEmpty) {
  ...
  Widget _buildImageLayer(
    ...
    String? mappedImg;
    final bool isBg =
        (layer['is_background'] == true || layer['is_background'] == 1);

    // Map AI injected images
    if (widget.aiData != null && widget.aiData!['_image_mappings'] != null) {
      final Map<String, dynamic> mappings = widget.aiData!['_image_mappings'];
    ...
    final bool isFrameSlot =
        lname.startsWith('image') && !lname.contains('icon');
    ...
    if (isLocal) {
      ...
      final bool isPlaceholder =
          (layer['is_placeholder'] == true || layer['is_placeholder'] == 1);
      ...
      if (layer['_businessKey'] == 'logo' ||
          lname.contains('logo') ||
          lname.contains('sticker') ||
          layer['isUserAdded'] == true) {
        fit = BoxFit.contain;
      } else if (isFrameSlot) {
        fit = BoxFit.cover; // Photo slots use cover scaling
      } else if (isFlatBg || isShapeBg || isPlaceholder) {
        fit = BoxFit
            .cover; // All backgrounds (flat or shaped) and placeholders should fill the area
      } else if (lname == '_frame_bg' ||
          lname == '_frame' ||
          lname == 'frame' ||
          layer['_is_frame_layer'] == true) {
        fit = BoxFit
            .fill; // Stretch frame components to exactly match computed scales and overlap 100%
      } else {
    ...
    // ══ DIAGNOSTICS ══
    debugPrint(
      '[IMG_LAYER] name="$lname" pathType=$pathType '
    ...
    final bool isSmallAsset =
        (nativeW > 0 && nativeW < 50) || (nativeH > 0 && nativeH < 50);

    Color? tintColor;
    List<Color>? gradientColors;
    final String _lowName = lname.toLowerCase();
    final bool _isContactOrSocial = [
      'phone',
      'email',
      'web',
      'address',
      ...
      'mail',
      'location',
    ].any((k) => _lowName.contains(k));

    if (_isContactOrSocial) {
      debugPrint(
        '[TINT] "$lname" tint_color=${layer['tint_color']} _businessKey=${layer['_businessKey']} is_shape=${layer['is_shape']} pathType=$pathType',
      );
    }
    ...
    final bool isRasterizedShape =
        (layer['is_shape'] == true || layer['is_shape'] == 1) &&
        pathType == 'TEMPLATE_ASSET';
    final bool skipRasterTint =
        isRasterizedShape &&
    ...
    if (layer['tint_color'] != null && !skipRasterTint) {
      ...
      debugPrint(
        '[TINT] "$lname" SKIPPED — rasterized shape v1, color baked in PNG',
      );
    }
    ...
    final bool isShapeLayer =
        layer['is_shape'] == true || layer['is_shape'] == 1 || isIcon;
    final bool hasInjectedImage =
        (pathType == 'API_INJECTED' ||
        pathType == 'BASE_IMG' ||
        pathType == 'AI_MAPPED');
    bool clipOval =
        !isShapeLayer &&
        hasInjectedImage &&
        (_lowName.contains('ellipse') ||
            _lowName.contains('circle') ||
            _lowName.contains('round'));

    Widget imgWidget = _buildImage(
      finalUrl,
      fit,
      radius,
      isSmall: isSmallAsset,
      tintColor: tintColor,
      ...
      clipOval: clipOval,
      isFrameBackground:
          lname == '_frame_bg' || lname == '_frame' || lname == 'frame' || isBg,
    );
    ...
    if (_isContactOrSocial &&
      ...
      if (_lowName.contains('phone') ||
          _lowName.contains('call') ||
          _lowName.contains('mobile')) {
        fallbackIcon = Icons.phone;
      } else if (_lowName.contains('email') || _lowName.contains('mail')) {
        fallbackIcon = Icons.email;
      } else if (_lowName.contains('web') ||
          _lowName.contains('globe') ||
          _lowName.contains('website')) {
        fallbackIcon = Icons.language;
      } else if (_lowName.contains('location') ||
          _lowName.contains('address') ||
          _lowName.contains('map')) {
      ...
      if (fallbackIcon != null) {
        debugPrint(
          '✅ [IMG_LAYER] Using native Flutter Icon fallback for broken raster icon "$lname"',
        );
        imgWidget = SizedBox(
          width: nativeW * scale,
          height: nativeH * scale,
          child: FittedBox(
            fit: BoxFit.contain,
            child: Icon(fallbackIcon, color: tintColor ?? Colors.black),
          ),
        );
      }
    }
    ...
    if (layer['mask_layer_id'] != null) {
      ...
      debugPrint(
        '[MASK_DIAG] maskShapeLayer found=${maskShapeLayer.isNotEmpty}',
      );

      imgWidget = _buildImage(
        finalUrl,
        BoxFit.cover,
        null,
        isSmall: false,
      ...
      if (maskShapeLayer.isNotEmpty &&
          (maskShapeLayer['src'] != null ||
              maskShapeLayer['_fallback_src'] != null)) {
        final String maskSrc =
            maskShapeLayer['src'] ?? maskShapeLayer['_fallback_src'] ?? '';
        final String maskUrl = _resolveAssetUrl(maskSrc);

        debugPrint('[MASK_DIAG] maskSrc="$maskSrc" → maskUrl="$maskUrl"');

        imgWidget = CustomImageMaskWidget(imageUrl: maskUrl, child: imgWidget);
        debugPrint('[MASK_DIAG] ✅ CustomImageMaskWidget applied for "$lname"');
      } else {
        debugPrint(
          '[MASK_DIAG] ⚠️ Mask shape has no src — cannot apply alpha mask',
        );
      }
      debugPrint('[MASK_DIAG] ═══════════════════════════════════════');
    }
  ...
  Widget _buildImage(
    ...
    Color? tintColor,
    List<Color>? gradientColors,
    String gradientDir = 'vertical',
    bool isLocal = false,
    bool flipX = false,
    bool clipOval = false,
    bool isFrameBackground = false,
  }) {
    if (url.isEmpty) {
    ...
    debugPrint(
      '[DIAGNOSIS_CANVAS] Layer ID: Unknown (inside _buildImage)',
    ); // Optional, since _buildImage doesn't take layerId
          ...
          .addListener(
            ImageStreamListener(
              (ImageInfo info, bool syncCall) {
                ...
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Image Width: ${info.image.width}',
                );
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Image Height: ${info.image.height}',
                );
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Duration: ${endTime - startTime}ms',
                );
              },
              onError: (dynamic exception, StackTrace? stackTrace) {
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Image Load Failed: $url - $exception',
                );
              },
            ),
          );
        ...
        if (isSmall && kIsWeb) {
          ...
          imgWidget = Image.network(
            ...
            errorBuilder: (context, error, stackTrace) {
              ...
              if (url.contains('%20')) {
                ...
                debugPrint(
                  'Retrying small image with fallback URL: $fallbackUrl',
                );
          ...
          imgWidget = CachedNetworkImage(
            ...
            imageBuilder: (context, provider) {
              ...
              debugPrint(
                '[DIAGNOSIS_CANVAS] paint() equivalent (imageBuilder)',
              );
              debugPrint('[DIAGNOSIS_CANVAS] frame id: $frameId');
            ...
            progressIndicatorBuilder: (context, url, progress) {
              if (isFrameBackground) {
                ...
                debugPrint(
                  '[DIAGNOSIS_CANVAS] progressIndicatorBuilder called',
                );
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Timestamp: ${DateTime.now().millisecondsSinceEpoch}',
                );
                debugPrint(
                  '[DIAGNOSIS_CANVAS] Download Progress: ${progress.progress}',
                );
                debugPrint(
                  '[DIAGNOSIS_CANVAS] --------------------------------------------',
                );
  ...
  Widget _buildVectorShape(
    ...
    if (layer['stroke'] != null &&
        layer['stroke'].toString() != 'null' &&
        layer['stroke'].toString() != 'none') {
  ...
  Gradient? _fabricGradientToFlutter(
    Map<dynamic, dynamic> gradObj,
    double w,
    double h,
  ) {
    try {
      ...
      if (coords == null || colorStops == null || colorStops.isEmpty)
        return null;
  ...
  Widget _buildIconLayerV8(
    ...
    String offlineSvg = '';
    if (layer['_source_meta'] is Map &&
        layer['_source_meta']['originalSvg'] != null) {
    ...
    if (offlineSvg.isNotEmpty) {
      debugPrint(
        "offlineSvg starts with: ${offlineSvg.substring(0, offlineSvg.length > 200 ? 200 : offlineSvg.length)}",
      );
      debugPrint(
        "offlineSvg contains viewBox: ${offlineSvg.contains('viewBox')}",
      );
    }
    ...
    if (iconData == null) {
      ...
      double size =
          (safeDouble(layer['size']) ?? (nativeH > 0 ? nativeH : 24.0)) * scale;
      ...
      if (offlineSvg.isEmpty) {
        debugPrint(
          '🔴 [SVG LOAD] No offlineSvg found for icon: $iconName, attempting native fallback',
        );
        ...
        if (lowerName.contains('phone') ||
          ...
          fallbackIcon = Icons.email;
            ...
            lowerName.contains('globe') ||
            lowerName.contains('website')) {
          fallbackIcon = Icons.language;
        } else if (lowerName.contains('location') ||
        ...
        if (fallbackIcon != null) {
          debugPrint(
            '✅ [SVG LOAD] Using native Flutter Icon fallback for "$lname"',
          );
          return SizedBox(
            ...
            child: FittedBox(
              fit: BoxFit.contain,
              child: Icon(fallbackIcon, color: parsedColor),
            ),
          );
        }

        debugPrint(
          '🔴 [SVG LOAD] No native fallback found, falling back to image layer',
        );
        return _buildImageLayer(layer, lname, scale, nativeW, nativeH);
      }
      ...
      offlineSvg = offlineSvg.replaceAll(
        RegExp(r'fill="[^"]*"'),
        'fill="$hexColor"',
      );
      offlineSvg = offlineSvg.replaceAll(
      ...
      if (layer['opacity'] != null) {
        svgWidget = Opacity(
          opacity: safeDouble(layer['opacity']),
          child: svgWidget,
        );
      }
    ...
    debugPrint(
      '[ICON_DIAG] FontAwesome "$lname" → font_color=${layer['font_color']} color=${layer['color']} → resolved colorStr=$colorStr',
    );
    ...
    Widget iconWidget = FaIcon(iconData, color: iconColor, size: size);
...
class _CustomImageMaskWidgetState extends State<CustomImageMaskWidget> {
  ...
  void initState() {
    super.initState();
    debugPrint(
      '[MASK_WIDGET] initState — loading mask from: ${widget.imageUrl}',
    );
    _loadMaskImage();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ShaderMask(
      ...
      shaderCallback: (Rect bounds) {
        ...
        debugPrint(
          '[MASK_WIDGET] shader: maskSize=${maskW}x$maskH bounds=$bounds scaleX=$scaleX scaleY=$scaleY',
        );

        final Matrix4 matrix = Matrix4.identity()..scale(scaleX, scaleY, 1.0);

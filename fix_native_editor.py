...
loadNewFrameStart = content.find('  Future<void> loadNewFrame(Map<String, dynamic> newFrameJson) async {')

new_methods = """  static final Map<String, bool> _brightnessCache = {};
  ...
  Future<void> loadNewFrame(Map<String, dynamic> newFrameJson) async {
    try {
      ...
      try {
        ...
        if (baseImgUrl.isNotEmpty) {
          ...
          if (_brightnessCache.containsKey(baseImgUrl)) {
            ...
            debugPrint('[BRIGHTNESS] Using cached value for $baseImgUrl -> isDark: $templateIsDark');
          } else {
            ...
            if (resp.statusCode == 200) {
              ...
              if (data != null) {
                ...
                for (int i = startIdx; i < len(data); i += 4 * 10) {
                  int r = data[i], g = data[i+1], b = data[i+2];
                  totalLuminance += (0.299 * r + 0.587 * g + 0.114 * b);
                  sampleCount++;
                }
                if (sampleCount > 0) {
                  double avg = totalLuminance / sampleCount;
                  templateIsDark = avg < 128;
                  _brightnessCache[baseImgUrl] = templateIsDark;
                  debugPrint('[BRIGHTNESS] Bottom 30% Avg: $avg -> isDark: $templateIsDark');
                }
              }
            }
          }
      ...
      try {
        ...
        } else if (templateConfig['info'] != null) {
          if (templateConfig['info'] is String) {
            final parsedInfo = jsonDecode(templateConfig['info']);
            if (parsedInfo['ppi'] != null) docPPI = (parsedInfo['ppi'] as num).toDouble();
          } else if (templateConfig['info']['ppi'] != null) {
            docPPI = (templateConfig['info']['ppi'] as num).toDouble();
          }
        }
      } catch (e) {
        debugPrint('[PPI] Failed to parse info block: $e');
      }
...
new_content = content[:loadNewFrameStart] + new_methods
# fix the len(data) bug from earlier! It should be data.length
new_content = new_content.replace('len(data)', 'data.length')

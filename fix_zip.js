...
if(fs.existsSync(jsonPath)) {
    ...
    function flattenJSONLayers(arr) {
        let flat = [];
        for (let i = 0; i < arr.length; i++) {
            if (arr[i].type === 'group' && arr[i].children) {
                flat = flat.concat(flattenJSONLayers(arr[i].children));
            } else {
                if (arr[i].type === 'shape') {
                    arr[i].type = 'image';
                    arr[i].is_shape = true;
    ...
    data.layers = flattenJSONLayers(data.layers);
    
    fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2));
    console.log('Fixed JSON inside template dir');
    
    // Also re-zip it using powershell
    try {

...
$category = \App\Models\BusinessCategory::find(41); // Automotive
$mdContent = "# Automotive Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Supplier', 'Wholesaler', 'Retailer', 'Service', 'Services', 'Center', 'Mechanic', 'Garage'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Service',
                    'Quality ' . $baseName . ' Parts',
                    'Automotive ' . $baseName . ' Solutions'
                ];
            ...
            $mdContent .= "- **Product Type:** Auto Parts / Vehicle Service\n";
            $mdContent .= "- **Brand:** Top OEM Brands / Authorized Service Centers\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Supplier', 'Wholesaler', 'Retailer', 'Service', 'Services', 'Center', 'Mechanic', 'Garage'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Automotive.md', $mdContent);

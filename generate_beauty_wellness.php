...
$category = \App\Models\BusinessCategory::find(13); // Beauty & Wellness
$mdContent = "# Beauty & Wellness Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Salon', 'Spa', 'Clinic', 'Center', 'Studio', 'Parlour'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Service',
                    'Expert ' . $baseName . ' Treatment',
                    $baseName . ' Package'
                ];
            ...
            $mdContent .= "- **Product Type:** Beauty Service / Wellness Treatment\n";
            $mdContent .= "- **Brand:** Premium Salons / Local Specialists\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Salon', 'Spa', 'Clinic', 'Center', 'Studio', 'Parlour'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Beauty & Wellness.md', $mdContent);

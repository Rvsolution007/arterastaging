...
$category = \App\Models\BusinessCategory::find(36); // Books Stationery & Office Supplies
$mdContent = "# Books Stationery & Office Supplies Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Supplier', 'Wholesaler', 'Retailer'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Supplies',
                    'Quality ' . $baseName . ' Items',
                    'Office ' . $baseName . ' Accessories'
                ];
            ...
            $mdContent .= "- **Product Type:** Stationery / Book / Office Supply\n";
            $mdContent .= "- **Brand:** Popular Stationery Brands / Local Publishers\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Supplier', 'Wholesaler', 'Retailer'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Books Stationery & Office Supplies.md', $mdContent);

...
$category = \App\Models\BusinessCategory::find(18); // Consultants & Advisors
$mdContent = "# Consultants & Advisors Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Consultant', 'Consultants', 'Advisor', 'Advisors', 'Agency', 'Firm', 'Company'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Consultation',
                    'Expert ' . $baseName . ' Advisory',
                    $baseName . ' Strategy Session'
                ];
            ...
            $mdContent .= "- **Product Type:** Consulting / Advisory Service\n";
            $mdContent .= "- **Brand:** Certified Professional / Top Consulting Firm\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Consultant', 'Consultants', 'Advisor', 'Advisors', 'Agency', 'Firm', 'Company'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Consultants & Advisors.md', $mdContent);

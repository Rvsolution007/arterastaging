...
$category = \App\Models\BusinessCategory::find(21); // E-commerce & Online Business
$mdContent = "# E-commerce & Online Business Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Store', 'Platform', 'Portal', 'Site', 'App', 'Website'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Subscription',
                    'Digital ' . $baseName . ' Products',
                    'Exclusive ' . $baseName . ' Services'
                ];
            ...
            $mdContent .= "- **Product Type:** Digital Product / Online Service\n";
            $mdContent .= "- **Brand:** Online Platforms / Verified Sellers\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Store', 'Platform', 'Portal', 'Site', 'App', 'Website'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('E-commerce & Online Business.md', $mdContent);

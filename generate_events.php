...
$category = \App\Models\BusinessCategory::find(33); // Event & Wedding Services
$mdContent = "# Event & Wedding Services Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Service', 'Services', 'Planner', 'Planners', 'Agency', 'Company', 'Decorator'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Service',
                    'Custom ' . $baseName . ' Arrangement',
                    'Special ' . $baseName . ' Package'
                ];
            ...
            $mdContent .= "- **Product Type:** Event Management / Wedding Service\n";
            $mdContent .= "- **Brand:** Premium Agencies / Local Experts\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Service', 'Services', 'Planner', 'Planners', 'Agency', 'Company', 'Decorator'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Event & Wedding Services.md', $mdContent);

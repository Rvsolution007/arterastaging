...
$category = \App\Models\BusinessCategory::find(7); // Food & Beverage
$mdContent = "# Food & Beverage Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Restaurant', 'Cafe', 'Shop', 'Store', 'Stall', 'Center'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Special ' . $baseName . ' Combo',
                    'Premium ' . $baseName . ' Item',
                    'Fresh ' . $baseName
                ];
            ...
            $mdContent .= "- **Product Type:** Veg / Non-Veg / Vegan / Beverage\n";
            $mdContent .= "- **Brand:** Local / Franchise\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Restaurant', 'Cafe', 'Shop', 'Store', 'Stall', 'Center'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Food & Beverage.md', $mdContent);

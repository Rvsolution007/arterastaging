...
$category = \App\Models\BusinessCategory::find(19); // Industrial & Engineering
$mdContent = "# Industrial & Engineering Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Manufacturer', 'Supplier', 'Company', 'Dealer', 'Contractor', 'Engineering'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Equipment',
                    'Industrial ' . $baseName . ' Solutions',
                    'Heavy Duty ' . $baseName
                ];
            ...
            $mdContent .= "- **Product Type:** Industrial Machinery / Engineering Service\n";
            $mdContent .= "- **Brand:** Top OEM Brands / Leading Suppliers\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Manufacturer', 'Supplier', 'Company', 'Dealer', 'Contractor', 'Engineering'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Industrial & Engineering.md', $mdContent);

...
$category = \App\Models\BusinessCategory::find(4); // Local & Trade Services
$mdContent = "# Local & Trade Services\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Service', 'Services', 'Provider', 'Agency', 'Contractor', 'Company'], '', $type->name);
                $baseName = trim($baseName);
                
                $newProducts = [
                    'Professional ' . $baseName . ' Services',
                    'Expert ' . $baseName . ' Solutions',
            ...
            $mdContent .= "- **Product Type:** Professional Service / Trade Labor\n";
            $mdContent .= "- **Brand:** Independent / Local Service Provider\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Service', 'Services', 'Provider', 'Agency', 'Contractor', 'Company'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Local & Trade Services.md', $mdContent);

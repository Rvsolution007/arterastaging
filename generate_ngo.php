...
$category = \App\Models\BusinessCategory::find(29); // Ngo & Non-profit
$mdContent = "# Ngo & Non-profit Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Ngo', 'NGO', 'Foundation', 'Organization', 'Society', 'Charity', 'Trust'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Campaign',
                    $baseName . ' Donation Program',
                    'Special ' . $baseName . ' Cause'
                ];
            ...
            $mdContent .= "- **Product Type:** NGO Campaign / Charity Program\n";
            $mdContent .= "- **Brand:** Registered NGO / Trusted Foundations\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Ngo', 'NGO', 'Foundation', 'Organization', 'Society', 'Charity', 'Trust'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Ngo & Non-profit.md', $mdContent);

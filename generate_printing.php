...
$category = \App\Models\BusinessCategory::find(25); // Printing & Packaging
$mdContent = "# Printing & Packaging Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Service', 'Services', 'Company', 'Agency', 'Press', 'Studio'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Service',
                    'Custom ' . $baseName . ' Solutions',
                    'Bulk ' . $baseName
                ];
            ...
            $mdContent .= "- **Product Type:** Printing Material / Packaging Solutions\n";
            $mdContent .= "- **Brand:** Trusted Local Presses / Top Manufacturers\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Service', 'Services', 'Company', 'Agency', 'Press', 'Studio'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Printing & Packaging.md', $mdContent);

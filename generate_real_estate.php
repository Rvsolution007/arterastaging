...
$category = \App\Models\BusinessCategory::find(8); // Real Estate & Construction
$mdContent = "# Real Estate & Construction Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Agency', 'Broker', 'Builder', 'Company', 'Contractor', 'Consultant'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Projects',
                    'Expert ' . $baseName . ' Consultation',
                    $baseName . ' Services'
                ];
            ...
            $mdContent .= "- **Product Type:** Real Estate Property / Construction Service\n";
            $mdContent .= "- **Brand:** Trusted Developer / Top Agency\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Agency', 'Broker', 'Builder', 'Company', 'Contractor', 'Consultant'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Real Estate & Construction.md', $mdContent);

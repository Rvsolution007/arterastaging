...
$category = \App\Models\BusinessCategory::find(1); // Retail
$mdContent = "# Retail Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Gallery'], '', $type->name);
            ...
            $mdContent .= "- **Product Type:** Retail Goods / Daily Needs\n";
            $mdContent .= "- **Brand:** Various Popular Brands / Local Brands\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Store', 'Shop', 'Dealer', 'Gallery'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('retail.md', $mdContent);

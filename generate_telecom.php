...
$category = \App\Models\BusinessCategory::find(27); // Telecom & Internet
$mdContent = "# Telecom & Internet Category\n\n";
$addedCount = 0;
...
foreach ($subs as $sub) {
    ...
    if ($sub->has_business_type) {
        ...
        foreach ($types as $type) {
            ...
            if ($products->count() == 0) {
                // Generate products
                $baseName = str_replace(['Provider', 'Service', 'Services', 'Company', 'Operator', 'Network'], '', $type->name);
                $baseName = trim($baseName);
                if (empty($baseName)) $baseName = $type->name;
                
                $newProducts = [
                    'Premium ' . $baseName . ' Plan',
                    'Business ' . $baseName . ' Connection',
                    'High-Speed ' . $baseName
                ];
            ...
            $mdContent .= "- **Product Type:** Telecom Plan / Internet Connection\n";
            $mdContent .= "- **Brand:** Leading ISPs / Telecom Operators\n\n";
        }
    } else {
        ...
        if ($products->count() == 0) {
            // Generate products
            $baseName = str_replace(['Provider', 'Service', 'Services', 'Company', 'Operator', 'Network'], '', $sub->name);
            $baseName = trim($baseName);
...
file_put_contents('Telecom & Internet.md', $mdContent);

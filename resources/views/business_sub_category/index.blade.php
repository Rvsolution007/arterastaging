...
<div class="aim-container">
    ...
    <div class="aim-header">
        ...
        <div class="aim-header-right">
            ...
            <a href="{{ route('business-sub-category.create')}}" class="aim-btn aim-btn-primary">
    ...
    <div class="aim-search-bar">
        ...
        <input type="text" id="search_input" class="aim-search-input" placeholder="Search sub categories by name...">
    </div>
            ...
            <table class="aim-table">
                <thead>
                    <tr>
                        ...
                        <th class="sortable-col" data-col="name" style="cursor: pointer;">
                            Sub Category Details <i class="fa-solid fa-sort sort-icon text-muted" data-col="name"></i>
                        </th>
                        <th class="sortable-col" data-col="types_count" style="cursor: pointer;">
                            Linked Business Types <i class="fa-solid fa-sort sort-icon text-muted" data-col="types_count"></i>
                        </th>
                        <th class="sortable-col" data-col="products_count" style="cursor: pointer;">
                            Connected Products <i class="fa-solid fa-sort sort-icon text-muted" data-col="products_count"></i>
                        </th>
                        <th class="sortable-col" data-col="category_name" style="cursor: pointer;">
                ...
                <tbody id="table_body">
                    ...
                    <tr>
                        ...
                        <td>
                            <span class="badge-category" style="background: #e0e7ff; color: #4f46e5;">{{ $row->types_count }} Types</span>
                        </td>
                        <td>
                            <span class="badge-category" style="background: #ecfdf5; color: #10b981;">{{ $row->products_count }} Products</span>
                        </td>
                        <td>
                            <span class="badge-category">{{ $row->business_category ? $row->business_category->name : '--' }}</span>
                        </td>
                        <td>
                            <label class="switch">
                        ...
                        <td class="text-right">
                            <div style="display: flex; gap: 8px; justify-content: flex-end;">
                                <a href="{{url('admin/business-sub-category/'.$row->id.'/edit')}}" class="action-btn btn-edit" title="Edit">
                            ...
                            @if(Auth::user()->user_type != "Demo")
                            <form action="{{url('admin/business-sub-category/'.$row->id)}}" id="form_{{$row->id}}" method="post" class="d-none">
                                @csrf
                                @method('DELETE')
                            </form>
                            @endif
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content" style="border-radius: 16px; border: none;">
            <div class="modal-body" style="padding: 2rem; text-align: center;">
                ...
                <p style="color: #64748b; font-size: 0.9rem; margin-bottom: 2rem;">Are you sure you want to delete <strong id="bulk_count_text">0</strong> sub categories? This action cannot be undone.</p>
...
<div id="importModal" class="modal fade" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form action="{{ route('business-sub-category.import') }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="modal-body p-4">
                    <div class="form-group">
                        ...
                        <small class="text-muted mt-2 d-block">Required Format: <br><b>ID, Name, Category ID, Category Name, Linked Types, Status</b><br>Leave ID blank to create new, provide ID to update existing. (Linked Types column is ignored during import).</small>
                    </div>
                </div>
...
<script type="text/javascript">
    ...
    $('#export_btn').on('click', function(e) {
        ...
        var exportUrl = "{{ route('business-sub-category.export') }}";
        if (query) {
    ...
    let currentSortCol = new URLSearchParams(window.location.search).get('sort_col') || '';
    let currentSortDir = new URLSearchParams(window.location.search).get('sort_dir') || '';

    // Set initial sort icons based on URL
    if (currentSortCol) {
        $('.sort-icon[data-col="'+currentSortCol+'"]').removeClass('fa-sort text-muted')
            .addClass(currentSortDir === 'asc' ? 'fa-sort-up text-primary' : 'fa-sort-down text-primary');
    }
    ...
    $(document).on('click', '.sortable-col', function() {
        ...
        icon.removeClass('fa-sort text-muted').addClass(currentSortDir === 'asc' ? 'fa-sort-up text-primary' : 'fa-sort-down text-primary');

        var query = $('#search_input').val();
        var ajaxUrl = "{{url('admin/business-sub-category/search')}}?sort_col=" + currentSortCol + "&sort_dir=" + currentSortDir;
    ...
    $('#confirm_bulk_delete').on('click', function() {
        ...
        $.ajax({
            url: "{{ url('admin/business-sub-category/bulk-delete') }}",
            type: "POST",
                        ...
                        response.data.forEach(function(row) {
                            ...
                            var tr = `
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge-category" style="background: #e0e7ff; color: #4f46e5;">${row.types_count || 0} Types</span>
                                    </td>
                                    <td>
                                        <span class="badge-category" style="background: #ecfdf5; color: #10b981;">${row.products_count || 0} Products</span>
                                    </td>
                                    <td>
                                        <span class="badge-category">${row.category_name}</span>
                                    </td>
                                    <td>
                                        <label class="switch">
                                    ...
                                    <td class="text-right">
                                        <div style="display: flex; gap: 8px; justify-content: flex-end;">
                                            ...
                                            <form action="${deleteUrl}" id="form_${row.id}" method="post">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                        </div>
    ...
    $('#search_input').on('keyup', function() {
        ...
        searchTimeout = setTimeout(function() {
            fetchResults("{{url('admin/business-sub-category/search')}}", query);
        }, 300);
    });
    ...
    $(document).on('click', '.pagination a', function(e) {
        ...
        var ajaxUrl = "{{url('admin/business-sub-category/search')}}";
        ...
        if(currentSortCol && currentSortDir) {
            ajaxUrl += (page ? "&" : "?") + "sort_col=" + currentSortCol + "&sort_dir=" + currentSortDir;
        }
